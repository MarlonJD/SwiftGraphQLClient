...
- SwiftPM dependency: the legacy generated-client package in `kindred.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved`.
- Xcode products: the legacy runtime/API products in `kindred.xcodeproj/project.pbxproj`.
- Codegen config: the legacy JSON config.
- Codegen script: the legacy codegen shell script.
...
- The legacy operation client exposes only `fetch` and `perform`.
- The old normalized cache was constructed in runtime setup, but app code did not call store read/write/watch APIs.
- Query cache policy usage is effectively `networkOnly` plus test double signatures.
- The app already had a `URLSessionKindredGraphQLClient`, but generated operation materialization was backed by the legacy generated-client runtime.
...
4. Replace legacy operation-client implementations with `GraphQLClient` equivalents. Done in Kindred with `SwiftKindredGraphQLClient`.
5. Replace the legacy session refresher with a package-backed refresher using `RefreshSessionMutation`. Done in Kindred with `GraphQLSessionRefresher`.
...
11. Remove legacy package products and run final `rg` acceptance. Done in Kindred.
