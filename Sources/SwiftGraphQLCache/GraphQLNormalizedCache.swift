...
public actor GraphQLNormalizedCache: GraphQLOperationCache {
    ...
    public func read<Operation: GraphQLOperation>(_ operation: Operation) async throws -> GraphQLCachedResponse {
        ...
        let requiredFields = rootRequiredFields(from: Operation.selections)
    ...
    private func rootRequiredFields(from selections: [GraphQLSelection]) -> Set<String> {
        ...
        for selection in selections {
            ...
            case .inlineFragment(_, let selections):
                fields.formUnion(rootRequiredFields(from: selections))
            case .fragmentSpread:
                continue
            }
        }
    ...
    private func selectionMissingFields(
        ...
        case .object(let object):
            ...
            for selection in selections {
                ...
                case .inlineFragment(_, let nestedSelections):
                    missing.append(contentsOf: selectionMissingFields(in: value, selections: nestedSelections, path: path))
                case .fragmentSpread:
                    continue
                }
            }
        ...
        case .array(let values):
            return values.enumerated().flatMap { index, value in
                selectionMissingFields(in: value, selections: selections, path: path + [String(index)])
            }
        case .null, .bool, .int, .double, .string:
